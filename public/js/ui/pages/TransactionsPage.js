...
class TransactionsPage {
  ...
  removeAccount() {
    if(this.lastOptions){
      const sureToDelete = confirm('Вы действительно хотите удалить счёт?');
      if(sureToDelete){
        const data = {
  ...
  removeTransaction( id ) {
    const toDelete = confirm('Вы действительно хотите удалить эту транзакцию?');
    if(!toDelete){
      return
    }
    const data = {
  ...
  formatDate(date){
    ...
    return formatDate.toLocaleDateString('ru-RU', {
        ...
        year: 'numeric'}) 
      + ' в ' + 
      formatDate.toLocaleTimeString('ru-RU', {
        hour: 'numeric', 
        minute: 'numeric'});
  }
